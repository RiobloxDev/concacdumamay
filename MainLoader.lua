...
-- Tạo cửa sổ UI chính (PHẢI TẠO TRƯỚC KHI DÙNG)
...
-- ===== NÚT BẬT/TẮT UI MẶC ĐỊNH =====
local ToggleUI = {
    Enabled = true,
    Key = Enum.KeyCode.RightControl  -- Phím tắt: Ctrl phải
}

-- Hàm bật/tắt UI (SAU KHI ĐÃ CÓ WINDOW)
local function ToggleUIFunction()
    ToggleUI.Enabled = not ToggleUI.Enabled
    if ToggleUI.Enabled then
        Window:Show()
    else
        Window:Hide()
    end
...
game:GetService("UserInputService").InputBegan:Connect(function(input, gameProcessed)
    ...
    if input.KeyCode == ToggleUI.Key then
        ToggleUIFunction()
    end
end)
-- ===== KẾT THÚC NÚT BẬT/TẮT UI =====
